...
ggplot(data = county_data, aes(y = grade, x = county_data[[10]])) + 
  ...
  labs(x = "Crime Rate Per 100,000", 
       ...
       title = "Average grades vs crime rate per county per year") +
  theme_bw()
...
population vs crime rater per 100,000
(want to see if there is stronger correlation between county population and crime or stronger correlation for grades and crime)
```{r population vs crime per 100,000}
 library(scales)
population_crime <- lm(Population ~ county_data[[10]], data = county_data)
get_regression_table(population_crime)

ggplot(data = county_data, aes(y = Population, x = county_data[[10]])) + 
  geom_point() + 
  geom_smooth(method = "lm", se = FALSE) +
  ...
  labs(x = "Crime Rate Per 100,000", 
       ...
       title = "Total county population vs crime rate per county per year") +
  theme_bw()
...
```{r bootstrap for crime rate per 100,000}
...
boot_distrib_crime_rate <- boot_samp_county_data %>% 
  group_by(replicate) %>% 
  summarize(stat = mean(na.rm = TRUE, `Crime Rate per 100,000 Population`))
